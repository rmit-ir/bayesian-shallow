...
system_cis <- compute_all_system_cis(model3) 
...
system_cis <- system_cis %>% mutate(names = str_replace_all(names, fixed("_"), "\\_"))
...
p <- p + coord_cartesian(xlim=c(-0.25,0.25))
p <- p + scale_x_continuous(breaks=c(-0.2,0,0.2))
...
p2 <- p2 + geom_point(aes(x=`Q50`), size=1.5, shape=21, 
                    color="black", fill="#4daf4a")
p2 <- p2 + coord_cartesian(xlim=c(-0.25,0.25))
p2 <- p2 + scale_x_continuous(breaks=c(-0.2,0,0.2))
...
ggsave(p, file="s-bayesian-systems-round1-model3.pdf", width=2, height=3) 
